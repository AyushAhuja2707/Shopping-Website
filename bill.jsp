...
try{
  ...
  ResultSet rs2 = st.executeQuery("select * from users inner join cart on users.email = '"+email+"' and cart.status='bill'");
  while(rs2.next()){
...
%>
<h3>Online shopping Bill (Made By Ayush Ahuja)</h3>
<hr>
...
<h3>Product Details</h3>
  ...
  <% 
    ResultSet rs3 = st.executeQuery("select * from cart inner join product on cart.product_id=product.id and cart.email = '"+email+"' and cart.status='bill'");
    while(rs3.next()){
      sno = sno+1;
  %>
  <tr>
